...
<body class="hold-transition sidebar-mini layout-fixed">
                ...
                <form role="form" id="myform" method="post" action="#" enctype="multipart/form-data">
                  ...
                  <div class="card-body">
                    ...
                    <div class="row">
                      ...
                      <div class="col-sm-3">
                        ...
                        <div class="form-group">
                          ...
                          <select name="brand_id" class="form-control">
                            <option value="">Select</option>
                            <?php
                            $query_cat = mysqli_query($connection, "SELECT * FROM `tbl_brand`") or die(mysqli_error($connection));
                            while ($row_cat = mysqli_fetch_array($query_cat)) {
                              if ($row_cat["brand_id"] == $row_list["brand_id"]) {
                                $select = "selected";
                              } else {
                                $select = "";
                              }
                            ?>
                              <option value="<?php echo $row_cat["brand_id"]; ?>" <?php echo $select; ?>> <?php echo $row_cat["brand_name"]; ?></option>
                            <?php } ?>
                          </select>
                        </div>
                      </div>
                      <!-- <div class="col-sm-3">
                        <div class="form-group">
                          <label>Is Seller All?</label>
                          <select name="is_seller_all" id="is_seller_all" onchange="display_seller()" class="form-control">
                            <?php
                            $is_seller_all = $row_list["is_seller_all"];
                            if ($is_seller_all == "1") {
                              $s1 = "selected";
                              $s2 = "";
                            } else {
                            ...
                            ?>
                            <option value="">Select</option>
                            <option value="1" <?php echo $s1; ?>>Yes</option>
  ...
  if (isset($_POST['update'])) {
    ...
    if ($count > 0) {
      ...
      // $is_seller_all = mysqli_real_escape_string($connection, $_POST['is_seller_all']);
      $is_seller_all = 1;

      $queryupdate = mysqli_query($connection, "update $table set `brand_id`='{$brand_id}',`is_seller_all`='{$is_seller_all}',`product_details`='{$product_details}',`product_price`='{$product_price}',`product_quantity`='{$product_quantity}',`product_name`='{$product_name}',`category_id`='{$category_id}',`product_image`='{$cimg}' where $primary_key='{$id}'") or die(mysqli_error($connection));
